...
### Harness — tool-call census (2026-08-25, harness/scripts only, no product change)
- **Tool-call success is measurable.** A shared census fixture (`e2e/fixtures/toolCallCensus.ts`) taps `subscribeMissionEvents` in all four proof-matrix lanes and counts observed/executed/refused/failed calls plus receipt-classified vacuous and intentional-no-op work, reusing the reporter's `classifyToolReceiptWork` and refusal-bucket vocabulary. Page collects, Node classifies; every failure mode (unarmed, lossy replay, harvest death) degrades to UNKNOWN, never zero. Survives the interrupted-continuation lane's plugin restarts via segmented re-arm with replay + id de-dup.
- **Four fabricated-zero seats removed.** The reporter's `toolCalls ?? 0` for scenario-less lanes (the CSV's explicit `observed=0` rows), the all-keys-at-0 refusal-bucket seed (six fake zeros on every summary-sourced row), the proof matrix treating record *existence* as knowledge (now falls through to graph mining / none), and the protected-release projection's `null → 0`. Unknown now prints blank everywhere; coverage will correctly look lower until census rows land. Historical CSV rows are untouched. The lone lane that did report a number (`daily-use-research`) feeds an evidence count, not a call count — it keeps its fingerprinted counter, and the census's true count lands beside it in `toolCallsObserved`.
- **Reds are classified from the Playwright sidecar report.** The exclusive runner's `--reporter` flag keeps assertion/diagnostic text off stdout, so every red fell to budget-exempt `process:matrix_unclassified` and cells looped past their attempt budgets (7× interrupted-continuation at 7944971). Classification now also reads the per-attempt `*.playwright-report.json`; the confirmed continuation deadlock signature classifies as `product:final_only_graph_deadlock` and consumes budget.
- **Summary freshness is strict.** The proof matrix compares the run summary's mtime against an exact pre-launch snapshot instead of a 5-second wall-clock grace that could admit the previous attempt's summary; the reporter also writes a durable mirror (`proof-matrix-state/daily-use-run-summary.latest.json`, temp-then-rename) that telemetry may read but gates never do.
- **Model-tier benchmark runner** (`scripts/run-model-tier-benchmark.mjs`): drives the proven cells through the exclusive runner with caller-chosen models (the matrix's own model is pinned), appending honest rows to the eval CSV with `data_source=model-tier-benchmark`. Small-model tier bar = research + vault-recall + code-delivery green in one sweep.
- Shared nullable-count helpers moved to `scripts/honest-counts.mjs` (one authority; a source guard blocks re-inlining).
