...
- UI: native Obsidian `ItemView`, vanilla DOM APIs, and `styles.css`. The plugin intentionally does not use React, Vue, Svelte, Next.js, LangChain, cloud services, vector databases, or a hosted backend. Browser interaction and experience memory are gated behind an optional localhost companion service under `companion/`.
- Model API: a provider-agnostic internal `ModelClient` interface with Ollama-compatible and GPT/OpenAI-compatible Chat Completions adapters. The runner uses only internal message/tool-call types; provider adapters translate HTTP bodies, streaming deltas, tool calls, tool-result transcripts, auth, and errors. No SDK dependency is required.
...
`main.ts` owns plugin lifecycle. On load it reads persisted settings, bounded chat history, and the research-memory index, creates the semantic index service, registers the `agentic-researcher-view`, adds the ribbon icon, adds commands including `Open Agentic Researcher`, `Rebuild Semantic Vault Index`, and `Open Semantic Vault Index`, and registers the settings tab. It also registers debounced vault listeners for markdown create/modify/delete/rename events so the derived semantic index can be updated without a background daemon.
...
`src/agent/reflex/` owns the optional agentic reflex layer. `AgenticReflexController` combines `intentRouter`, `actionScorer`, `progressMonitor`, and `completionEvaluator` into one transient per-run signal. When enabled, it uses the existing local semantic embedding provider to compare the mission against embedded intent prototypes, scores candidate next actions, detects repeated tool loops, and checks whether required evidence or write receipts are present before final answer. Reflex output can add only safe read/context routing hints for vault, semantic, graph, web, and word-count work. It cannot grant write, delete, browser, credential, upload, purchase, or companion authority, and explicit mutation/delete missions ignore reflex-added read routing. Prompt vectors and reflex classifier outputs are not persisted.

`src/agent/missionScope.ts`, `src/agent/intent/structuredIntent.ts`, `src/agent/missionAcceptance.ts`, `src/agent/missionLedger.ts`, `src/agent/missionEvidence.ts`, and `src/agent/missionResume.ts` own the explicit autonomy layer. Mission scope is derived during classification and tightens the existing tool allow-list without exposing any tools that routing would otherwise block. Structured intent adds deterministic facets for primary task type, evidence needs, write target, mutation kind, destructive intent, confidence, and ambiguities; it is emitted to Run Details and injected as a compact system hint for the model. Mission acceptance checks required writes, receipts, failed/pending operation goals, explicit web/vault/word-count evidence needs, and empty final output before accepting completion. The ledger module writes a human-readable `Agent Runs/<runId>.md` record with one machine-readable fenced JSON block, evidence and receipt ids, blockers, next actions, route, loop budget, acceptance status, remaining actions, resume count, and last safe step. Evidence conversion turns successful source/vault/artifact tool results and write receipts into compact resumable records; `web_search` is medium-confidence source evidence and `web_fetch` is high-confidence source evidence. Resume loading accepts `continue run <runId>` and falls back to the latest ledger for generic continue/resume prompts.
...
`src/model/OpenAICompatibleClient.ts` owns request and response normalization for GPT/OpenAI-compatible Chat Completions endpoints. It maps internal `system`, `user`, `assistant`, and `tool` messages to Chat Completions messages; maps internal tool definitions to `tools: [{ type: "function", function: ... }]`; sends `tool_choice: "auto"` only when tools are available; parses non-streaming `message.tool_calls`; parses SSE `data:` streaming chunks; emits `delta.content` to the runner; and accumulates streamed `delta.tool_calls[*].function.arguments` before returning normalized `ModelToolCall[]`. It maps 401/403 to auth errors, 429 to rate-limit errors, and other provider failures to `ModelClientError`.

`src/model/toolTranscript.ts` owns provider-valid tool transcript assembly. `appendToolTranscript` ensures every internal tool call has an id, appends the tool result with `role: "tool"`, `toolName`, `toolCallId`, and serialized content, and adds a synthetic assistant tool-call message for runner-owned fallback tools. This keeps OpenAI-compatible transcripts valid without making `AgentRunner` understand provider-specific message rules. Ollama continues to receive the same internal transcript and ignores fields it does not need.

`src/model/createModelClient.ts` selects the configured adapter from `settings.modelProvider`. `ollama` returns `OllamaClient`; `openai_compatible` returns `OpenAICompatibleClient`. The shared transport still uses Obsidian `requestUrl` for normal requests and a hybrid streaming transport for streaming responses. Streaming first tries `fetch`; on desktop it can fall back to Node `http` or `https`. The Obsidian `requestUrl` lookup is lazy so pure model factory tests do not require the Obsidian runtime.

`src/tools/createToolRegistry.ts` combines vault tools, vault-index tools, semantic search tools, web tools, source-opening tools, code tools, design tools, and companion-backed browser/memory tools into a `DefaultToolRegistry`. The registry exposes model tool definitions and executes only known tools.
...
`src/tools/semanticSearchTools.ts` owns `semantic_search_notes`, `inspect_semantic_index`, and `rebuild_semantic_index`. `semantic_search_notes` is the read-only conceptual vault search tool for prompts such as `What do my notes say about MCP?`, theme/concept searches, and related-note questions where exact wording may differ. It rejects unsafe folders through the same vault-relative path policy, returns compact citation-ready results, and never returns raw vectors or full note bodies to the model. Its search order is: use fresh persisted semantic index vectors, fall back to live FastEmbed chunk embedding, then fall back to lexical/title/tag/path scoring with `fallbackUsed: true` when Python, FastEmbed, model download, or embedding calls fail. Results include retrieval coverage from `src/agent/retrievalCoverage.ts` with mode, considered/read/skipped counts, truncation, fallback use, confidence, and reasons so Run Details can distinguish exact, sampled, indexed, and fallback evidence. `inspect_semantic_index` is read-only and returns freshness, concepts, paths, and short evidence summaries for conceptual navigation. `rebuild_semantic_index` is a write-class maintenance tool and is only exposed when the user explicitly asks to rebuild, refresh, repair, create, regenerate, or update the semantic index.

`src/embeddings/semanticIndex.ts` owns the persistent derived semantic vault index. It writes a human-readable Markdown map to `Agent Memory/Semantic Vault Index.md` and machine JSON with local vectors to `Agent Memory/semantic-vault-index.json`. The Markdown file contains index metadata, concepts, paths, headings, tags, chunk counts, and bounded snippets, but no vectors. The JSON file stores `version`, `model`, `dim`, chunking settings, `indexedAt`, note metadata, chunk snippets, text hashes, and vectors when `semanticIndexPersistVectors` is enabled. The service treats both files as derived state: if JSON is missing, invalid, stale, disabled, or incompatible with the current model/dimension/chunk settings, search marks the index stale and `semantic_search_notes` continues to the live embedding path. Staleness is checked against each indexed file's vault path, mtime, size, and vector length, and against newly indexable files up to the configured file cap. Indexing skips non-markdown files, unsafe paths, `.agent-backups`, `.obsidian`, `.trash`, `trash`, `Agent Runs`, and the index files themselves. It uses the same 300/500/700 token min/target/max chunking and 80-token overlap as live semantic search.

In production, `main.ts` injects `createPythonFastEmbedProvider` through `ToolExecutionContext.semanticEmbeddingProvider`; tests and e2e can inject deterministic providers. The provider runs an on-demand local Python helper with JSON over stdin/stdout, no shell interpolation, a timeout, and bounded stdout/stderr. The helper uses FastEmbed `TextEmbedding` with `nomic-ai/nomic-embed-text-v1.5-Q`, prefixes documents as `search_document:` and queries as `search_query:`, downloads/verifies the model on first initialization, applies Matryoshka truncation to 512 dimensions by default, normalizes full and truncated vectors, and returns plain float arrays only to plugin code. There is still no vector database, HTTP embedding service, backend service, or background daemon.
...
Research memory also lives in `src/tools/vaultTools.ts`. `append_research_memory` writes durable topic memory as markdown under the active-note project folder, defaulting to `Agent Memory/Research`, and updates the project-local memory index with topic, path, keywords, last-updated time, optional confidence, source paths, source URLs, content hash, and update count. Duplicate appends with the same content hash are no-op receipts instead of repeated note text. `read_research_memory` reads a topic note by slug/index entry, and `search_research_memory` ranks the index by topic/keyword overlap and returns bounded excerpts from matching memory notes. `review_research_memory` reports duplicate and stale index entries, `compact_research_memory` replaces one memory note with a compacted summary after backing up the original, and `delete_research_memory_entry` trashes one memory note after backup and removes its index entry only for explicit delete/remove/trash memory prompts. The markdown note is the source of truth; the JSON index is only the recall map.
...
`src/agent/ToolContracts.ts` defines the shared public contracts for the web-acting upgrade: tool risk/status, artifact references, safety decisions, tool calls, tool receipts, tool results, browser inputs and observations, clickable candidates, memory write/search inputs, and memory search results. The contract is intentionally receipt-heavy so the plugin can prove work through observable records instead of hidden chain-of-thought.

`src/agent/SafetyPolicy.ts` is the browser safety gate. It blocks browser automation when Obsidian is not on desktop, browser tools are disabled, or the companion health check fails. It rejects `javascript:`, `data:`, `vbscript:`, `file:`, credential-bearing URLs, and localhost/private-network targets. Browser open is low risk only after those checks. Browser click/type are medium risk unless high-risk labels, visible text, roles, hrefs, or typed text indicate passwords, login, checkout, payment, purchase, submit, upload, delete/remove, download, or executable/script downloads. Reversible high-risk click/type actions return `requires_approval` until the user grants explicit one-shot approval; credential, checkout, payment, purchase, upload, and executable/script patterns remain blocked even with approval.

`src/agent/CompanionClient.ts` is the timeout-backed HTTP client for the optional local companion. It exposes health, browser open/observe/click/type/keypress/scroll/screenshot/extract-markdown, and memory write/search methods. HTTP failures include response body text, and unit tests inject a deterministic fetch implementation so the client can be tested without a running companion.

`src/tools/companionTools.ts` registers the new companion-backed tools. Browser tools are exposed only when settings, desktop runtime checks, companion health, route intent, and `SafetyPolicy` allow them. Memory tools are exposed only when experience memory is enabled and route intent makes memory useful. Blocked calls return structured tool results with visible safety decisions rather than silently disappearing, and approval-required calls include a short-lived approval id, risk, reason, and one-shot expiration so the UI can ask for a bounded user grant.

The optional local companion lives under `companion/`. It is a localhost-only FastAPI service with Playwright browser control, screenshots under `companion/data/screenshots`, readable HTML-to-Markdown extraction, a SQLite/FTS5 memory store, and a static Ruffle host for bounded SWF experiments. The companion does not make agent decisions; the plugin remains the orchestrator and safety gate. Runtime data, virtual environments, caches, databases, and screenshots are gitignored except `companion/data/.gitkeep` and `companion/data/README.md`.
...
`src/design/jsonCanvas.ts`, `src/design/layout.ts`, and `src/design/svgDesign.ts` own native design artifact generation. JSON Canvas helpers validate `.canvas` files against Obsidian-compatible node and edge structure, including ids, node geometry, node types, link URLs, and edge references. Layout helpers can build row, column, grid, user-flow, software-architecture, UI-flow, logistics-system, service-blueprint, and project-ideation canvases from model-provided items, add a title node when the model supplies a canvas title, group flow/system items into lane nodes, and create sequence or explicit labeled edges. SVG helpers render escaped diagram and wireframe primitives including rectangles, text, lines, arrows, circles, ellipses, decision diamonds, and database cylinders without scripts, event handlers, or `javascript:` URLs.

`src/tools/designTools.ts` owns `create_design_canvas`, `create_svg_design`, and `create_design_package`. The package tool delegates to `src/agent/design/CanvasWriter.ts` and `MarkdownBriefWriter.ts` to create one `.canvas` artifact and one companion markdown brief for UI flows, logistics systems, service blueprints, architecture maps, project ideation boards, and mind maps. Design packages default to `Design Packages/`, slugify the title, normalize and validate vault-relative folders, reject traversal, choose unique no-overwrite paths, verify Canvas JSON, and return artifact refs and write receipts for both files. The existing Canvas/SVG tools still require explicit draw/design/canvas/diagram/wireframe/SVG intent, reject existing output paths, create parent folders only when requested or by default, write through Obsidian vault APIs, read the artifact back, verify it, optionally open it in Obsidian, and return path, byte count, and node/edge or shape counts. During artifact creation they report progress through `ToolExecutionContext.reportProgress`, so Run Details streams design planning, validation, and write steps while the tool is running.
...
Mission ledgers now live in the same `Agent Runs/<runId>.md` file. `AgentRunner` creates the ledger after `RunPlan` and `LoopBudgetPlan` exist, then updates it after tool successes, write receipts, blockers, user stop, completion, and budget stops. The ledger also records milestone entries for stages such as `plan`, `gather`, `browser_observe`, `browser_act`, `synthesize`, `verify`, `write_save`, `memory_reflection`, and `next_action`; each milestone can carry a summary, decision, receipts, artifacts, evidence refs, errors, and next action. The markdown stays readable through a Mission Summary section, while the fenced JSON block is the parseable resume source. Old ledger JSON without `milestones` parses with an empty milestone list so resume stays backward compatible. A prompt such as `continue run run-123` loads that specific ledger as transient system context with incomplete tasks, evidence summaries, blockers, milestones, and next actions. Generic `continue`, `keep going`, or `resume` still falls back to the latest ledger/checkpoint. None of this resume text is persisted into chat history.
...
3. If `agenticReflexEnabled` is on, the runner evaluates a transient reflex signal before route planning. Reflex can upgrade only safe read/context hints and is ignored for explicit mutation/delete routing.
4. The runner builds runtime context including date/time, settings-derived model options, bounded chat history, current-note context when relevant, and tool definitions filtered by routing plus scope.
5. If current-note observation is required, the runner executes `read_current_file` before the first model planning call.
6. After route and loop budget planning, the runner creates a mission ledger under `Agent Runs/<runId>.md` when vault APIs are available.
7. The model receives the system prompt, mission context, scope summary, optional current-note context, optional resume context, optional recent chat history, and exact available tool definitions.
8. The model may return tool calls. The plugin validates that each requested tool is allowed for the mission before execution.
9. Tool results are serialized through the internal transcript helper and returned to the model for the next step; successful source/vault/artifact/search results are also compacted into ledger evidence when possible.
10. Before final answer, the reflex completion evaluator can require missing evidence or receipts only when reflex is enabled. Write tools stop the loop after requested write goals complete, emit receipts, and add receipt evidence to the ledger. Chat-only runs stop on final answer. The loop also stops on clarification, user stop, error, or step budget.

Each mission gets a `RunPlan` before the loop starts. The run plan records `route`, `maxStepsForRun`, `slowPathReason`, `expectedTimeClass`, `thinking`, `allowedTools`, and whether the English guard is required. `MAX_AGENT_STEPS = 60` is the hard safety ceiling, and the persisted `maxAgentSteps` setting is clamped to that ceiling. The runner then derives a `LoopBudgetPlan` from `src/agent/loopPlanner.ts`: generated sourced essays get a source-tool budget plus the finalization reserve, direct writeback gets one synthesis pass, artifact prompts get only the tool room they need, and ordinary chat answers use a short one-or-two-step path. Long/deep research, broad strategy, multi-source synthesis, browser-learning, and design-package prompts route into a longer grounded workflow with up to 60 steps while preserving `FINALIZATION_RESERVE_STEPS = 4` for verification, artifact write, memory/reflection write, and final response. Reflex hints can move ambiguous safe read prompts into vault, semantic, graph, web, or word-count routes; they do not change explicit write/delete routes or expose mutation tools. Prompts that explicitly ask to complete a specific number of model/agent/planning/loop steps override the route estimate up to the configured max and hard cap so Run Details and the chat CRT loader show `Agent step N of max N` instead of the default grounded-workflow budget. Planning output includes `tool_budget=...` and `finalization_reserved=4` so Run Details shows the adaptive budget instead of only the hard cap. The mission ledger stores the same budget fields plus expected tools and milestones so resume can tell whether the previous run was still gathering evidence, observing a browser page, writing artifacts, or ready to synthesize. Explicit design, design-package, code execution, HTML preview, browser automation, memory, and source-opening prompts route as artifact-like grounded workflows so the model has room to plan, call tools, verify outputs, and summarize. The `prefetched_vault_answer` route handles simple folder-content questions by running `inspect_vault_context` locally first, then making a single model call with `think` omitted and no model-visible tools. The `prefetched_vault_writeback` route uses the same local prefetch, then streams the synthesized markdown through current-note append writeback with no model-visible tools. Thinking is route-specific: quick local/direct/prefetched paths disable thinking, while grounded workflows and prompts such as `deep research`, `long research`, `in-depth research`, and `deep dive` use the settings-derived thinking mode when the selected model supports it.

During model/tool loops, `AgentRunner` maintains a transient `LoopLedger` with successful tools, failed tools, repeated tool calls, whether required tools are satisfied, whether a finalization pass is reserved, and whether a write completed. It also maintains mission operation goals keyed by concrete capabilities: read current note, web search, web fetch, current-note title, current-note content, current-note replace, section edit, path create, path append, path replace, path move, path delete, and current-note delete. Model-requested and runner-owned tool calls go through the same observed execution path so tool start/done events, metrics, serialized tool messages, receipts, ledger evidence, and operation-goal status are recorded consistently. The observed execution path calls `appendToolTranscript`, which supplies fallback tool-call ids and inserts the synthetic assistant tool-call turn required by OpenAI-compatible providers when the runner executes a fallback tool. After each tool result, reflex reevaluates progress and loop risk from transient action/evidence/receipt records. A write-like tool no longer completes the run by itself; the runner only stops after every requested operation goal is either not requested or done. This lets a single mission retitle a note and then write content, or create/update/delete multiple explicit paths without stopping after the first mutation.

`src/agent/loopDecision.ts` lets the runner own loop control: once required web/vault/design tools are satisfied, the runner removes tools and forces a final no-tool synthesis/writeback; if the same tool call repeats without progress, the runner gives one corrective prompt and then stops instead of burning all remaining steps. Source/citation/verification prompts require `web_search` and at least one `web_fetch` when `web_fetch` is available; simpler web-search summary prompts may finish after search context. If the model answers too early after the corrective prompt, `AgentRunner` can perform a deterministic read-only fallback: build a compact query from the mission, run `web_search`, fetch the first safe HTTP/HTTPS result with `web_fetch`, append both tool results to the model messages, and then force no-tool final synthesis or streamed note writeback. After any no-tool final answer, mission acceptance records pass/fail/needs-more-work into Run Details and the mission ledger. It only forces another model step when an actionable required write receipt, explicit web evidence, word-count check, or very explicit vault-before-answering requirement is still missing and the required tool is available; ordinary tool exposure alone is not treated as a mandatory acceptance item. The fallback is limited to web read tools; the runner does not auto-create, replace, move, or delete vault content without explicit user intent and validated tool routing. Vault traversal prompts keep the existing map/select/read policy: map folders or markdown files, select likely notes, then read content before synthesizing. This prevents source prompts from ending with `Stopped at safety limit` after the last `web_fetch` when enough context has already been gathered.

Long grounded loops emit live planning updates through `onPlanningStart`, `onPlanningDelta`, and `onPlanningDone`. The delta shows only observable routing metadata such as `Step 4/60`, `route=grounded_workflow`, `reason=needs_web_sources`, and the allowed tool names. Hidden model thinking is still discarded by `AgentView` and is not displayed or persisted. Runs that reach step 15 emit a long-run status warning. Runs checkpoint every five steps through `src/agent/checkpoints.ts`; if the run stops at the budget limit, the runner forces a final budget checkpoint so the partial progress is inspectable under `Agent Runs/`.
...
The runner creates one per-mission `AgentRuntimeCache` and passes it through `ToolExecutionContext`. It caches successful read-only tool results by stable tool name plus normalized arguments for `read_current_file`, `inspect_vault_context`, `inspect_vault_index`, `list_markdown_files`, `search_markdown_files`, `inspect_semantic_index`, `semantic_search_notes`, `read_markdown_files`, `read_file`, `count_words`, `web_fetch`, `get_note_graph_context`, `find_related_notes`, and `suggest_note_links`. Cache hits emit tool metrics with `cached=true` and do not call the registry again. `runtimeCache.semanticProfiles` stores per-run live semantic chunk profiles keyed by path, mtime, size, model, dimension, and chunk settings; those live vectors and chunks are discarded after the mission. The persistent semantic index is separate derived vault state under `Agent Memory/`.
...
The current-note streaming writer keeps a short safety/topic/language buffer and then flushes live chunks with lower thresholds than the older bulk buffer. After the initial safety buffer, it flushes at roughly 120 characters or 150 ms, with a 75 ms timer, so provider token-sized chunks can appear in the editor before the run completes. If a generated markdown draft starts with a leading H1, the streaming writer consumes that prefix before append output is released. It accepts normal H1s, H1s split across stream chunks, up to three leading spaces, and optional closing hashes such as `# Essay Title #`. It updates the existing note title slot through `retitleNoteMarkdown` against the latest active editor/body content rather than only the startup snapshot, then streams only the remaining body. The title helper also matches frontmatter `title:` case-insensitively and replaces decorated/indented first H1 lines. This turns `# Untitled` plus a generated `# Essay Title` into a single top H1 while preserving body text that arrived after writer setup. This does not rename the file; filename rename remains a separate explicit move/rename operation.

Vault and folder content questions are guarded against chat-only refusals. Prompts such as `What do the other notes in the other folders say?` and `gather details from the other folders` route through `prefetched_vault_answer`: the runner calls `inspect_vault_context` with `scope: "other_folders"`, adds the JSON result as system context, tells the model to cite vault-relative paths, and sends no tools to the model. Named-folder traversal prompts such as `traverse the 3 folders named Untitled, Untitled 1 and Untitled 2` pass `targetFolders` to `inspect_vault_context`, which matches vault-relative folder paths or folder basenames, reads bounded markdown descendants, excludes the active prompt note, prioritizes recent files before the per-run cap, and reports matched/unmatched targets. Its output includes retrieval coverage with exact or sampled mode, selected/read/skipped counts, truncation, confidence, and reasons. When the extracted page prompt also asks to stream or write the findings onto the current page, `prefetched_vault_writeback` runs the same local inspection first and then streams the answer into the current note with an append receipt. The prefetched vault JSON is also included in the relevance anchors for the single answer/writeback call so source-derived terms are not falsely blocked as off-topic. Topic-search prompts such as `What do my notes say about MCP?` expose `inspect_semantic_index` and `semantic_search_notes` alongside the normal search/batch-read tools, and the system prompt tells the model to inspect the semantic index when available, then call semantic search for ranked evidence while still preferring exact file/title/path tools for exact requests. Vault/folder routing wins over generic research verbs like `gather`, so those prompts do not expose web tools unless the user explicitly asks for web, sources, citations, latest/current outside information, or verification. If a grounded vault fallback is needed and the model tries to answer before any vault traversal tool has run, `AgentRunner` adds one corrective system message that tells it not to claim it cannot access the vault and to request a vault tool. If the next step still makes no tool progress, the run stops with a clear error and no vault files are changed instead of looping to the full step cap.
...
- The reflex layer is also a tightening-safe hint layer, not an authority layer. It can make ambiguous read missions choose safe vault, semantic, graph, web, or word-count context tools, but it cannot expose write, delete, browser, companion, credential, upload, purchase, or code-execution tools. Explicit mutation/delete prompts ignore reflex-added read routing and rely only on deterministic intent.
...
- Conceptual vault questions expose `inspect_semantic_index` and `semantic_search_notes` as read-only navigation/evidence tools. Exact path, title, heading, or filename requests still prefer `read_file`, `search_markdown_files`, `inspect_vault_index`, list tools, and explicit read tools. Delete, move, replace, and direct current-note write-only prompts do not expose semantic search or semantic index inspection. Explicit semantic index maintenance prompts can expose `rebuild_semantic_index`.
- Vault map/index prompts expose `inspect_vault_index`, which returns metadata only and does not read note bodies.
- Research-memory prompts expose `search_research_memory` and `read_research_memory`; explicit save/remember/persist prompts expose `append_research_memory`, explicit review/audit/cleanup prompts expose `review_research_memory`, explicit compact/summarize/merge prompts expose `compact_research_memory`, and explicit delete/remove/trash memory prompts expose `delete_research_memory_entry`.
...
- Word-count and length-check questions expose `count_words`, which returns count metadata only and never returns note content. The completion gate treats a premature answer to a word-count mission as incomplete when `count_words` is still available.
...
- Generated essays, how-tos, and explanations use streamed current-note writeback by default when an active note exists. Grounded planning runs that need sources, vault context, or other tools may still expose matching current-note write tools such as `append_to_current_file` or `replace_current_file` so the model can request web/read/write tools in the same run without hitting stale unavailable-tool errors. If no write tool is requested, the runner streams the final markdown into the active note.
...
- Browser tools are exposed only for explicit browser interaction, web navigation, click/type/scroll, live-page observation, workflow-learning, or browser-learning intent. They require desktop Obsidian, `browserToolsEnabled`, a healthy companion, HTTP/HTTPS URLs without credentials, non-private targets, and `SafetyPolicy` approval. Mobile disables browser actions. Each action is one observed tool step with a receipt, safety decision, observation/error, and mission-ledger milestone.
- Experience-memory tools are exposed only when `experienceMemoryEnabled` is on, the companion is healthy, and the prompt is a research, writing, browser-learning, design-package, continuation, or explicit memory task. Source memories include source metadata, memory writes are visible in Run Details, and semantic/procedural memories should be written only when evidence is strong.
...
- `create_design_canvas`, `create_svg_design`, and `create_design_package` are exposed only for explicit create/make/draw/generate/build/draft/render/save/write design intent. Generic design, diagram, map, user-flow, logistics-system, service-blueprint, project-ideation, and software-architecture prompts default to JSON Canvas or Canvas-plus-brief design packages unless the prompt clearly asks for SVG, wireframe, mockup, screen, layout, or UI design.
- Replace, edit, retitle, move, create, and delete tools require explicit intent.
- Template filling creates a new markdown note by default and does not overwrite existing notes. Explicit `targetPath` wins. A configured `templateOutputFolder` is used next. When the output setting is blank, generated filled-template notes land in the active note's project folder, which is the active markdown file's parent folder, or the vault root when the active file is already at the root. If the model answers in chat for an explicit template fill/create request, the runner asks it to use the required template write tool.
...
- `defaultBrowserMissionMode`
- `agenticReflexEnabled`
- `agenticReflexDiagnosticsEnabled`
- `thinkingMode`
- `streamWritebackMode`
- `templateFolder`
...
Model settings default to `modelProvider: "ollama"`, `ollamaBaseUrl: "https://ollama.com/api"`, and the existing shared `model` field. Selecting `openai_compatible` uses `openAiCompatibleBaseUrl`, defaulting to `https://api.openai.com/v1`, plus `openAiCompatibleApiKey`. The settings UI shows one provider dropdown, provider-specific API key/base URL fields, and one shared model input so the runner can stay model-agnostic. `main.ts` normalizes invalid provider values back to `ollama`, trims base URLs, and preserves empty API keys for local unauthenticated endpoints.

Agentic reflex settings default to safe-off execution: `agenticReflexEnabled` is `false`, and `agenticReflexDiagnosticsEnabled` is `true`. With diagnostics on, `Run Details` shows the transient reflex intent, confidence, top scored action, progress score, loop-risk score, missing completion requirements, and applied reason. Disabling diagnostics does not change runtime routing; it only hides those debug fields from the model config panel.

Companion settings default to safe-off behavior: `companionBaseUrl` is `http://127.0.0.1:8765`, `browserToolsEnabled` is `false`, `experienceMemoryEnabled` is `false`, and `defaultBrowserMissionMode` is `supervised`. `main.ts` normalizes missing or invalid values on load, clamps `maxAgentSteps` to 60, and trims the companion URL. Browser tools and experience memory are not exposed merely because the companion exists; they also require the matching setting and health/desktop checks at tool-selection and execution time.
...
Semantic index persistence is separate derived state under the configured `semanticIndexFolder`, defaulting to `Agent Memory`. Defaults are enabled, 3000 ms debounce, 1000 files, 512 embedding dimensions, and persisted local vectors in JSON. Changing the model, dimension, chunking, or vector-persistence setting makes existing index JSON stale until rebuilt or updated. The settings UI exposes semantic search, embedding, chunking, Python/cache, and index maintenance options, but the model-facing tools still hide raw vectors. The four semantic chunk token inputs use a dedicated labeled settings grid instead of stacking unlabeled `Setting.addText` controls in one row, so the min/target/max/overlap fields remain visible and bounded in narrow Obsidian settings panes.

Mission ledgers are not chat memory and do not live under `Agent Memory`. They are per-run operational records in `Agent Runs/<runId>.md`. When loaded for resume, only a transient system-context summary is injected into the current model request. That summary includes incomplete tasks, evidence summaries, blockers, acceptance status, remaining actions, last safe step, resume count, and next actions, but it is not saved through `conversationHistory`.
...
- browser rows with current URL/title, screenshot path, visible text summary, clickable candidates, and page-state hints when companion observations are available
- action rows with tool names, safety decisions, receipt status, and errors
- milestone rows for plan/gather/browser/synthesize/verify/write-save/memory/next-action stages
- memory rows for searches and writes, including kind, confidence, source metadata, and evidence refs
- evidence and artifact rows for source notes, extracted markdown, screenshots, canvases, markdown briefs, receipts, and memory ids

Model Config reports context with separate terms:

- `provider`: `ollama` or `openai_compatible`, plus the active provider base URL in the compact diagnostic string
...
- `reflex_intent`, `reflex_confidence`, `reflex_top_action`, `reflex_progress`, `reflex_loop_risk`, `reflex_missing`, and `reflex_reason`: optional diagnostics emitted only when reflex output exists and diagnostics are enabled
...
Provider and reflex unit coverage lives in `tests/OpenAICompatibleClient.test.ts`, `tests/createModelClient.test.ts`, `tests/toolTranscript.test.ts`, `tests/agenticReflex.test.ts`, and the reflex-specific cases in `tests/AgentRunner.test.ts`. These tests verify OpenAI-compatible message/tool mapping, streaming SSE accumulation, provider factory selection, valid fallback tool-call ids, semantic reflex upgrades, mutation-safety blocking, progress loop detection, and completion evidence checks.
...
- runs isolated Playwright scenarios for clear-chat prompt typing, append, prior-assistant writeback, streaming writeback, folder traversal, semantic index inspection plus note search, reflex-assisted semantic routing, autonomous loop depths, checkpoint creation during long autonomous runs, prompt-on-page traversal, current-page replacement, explicit CRUD chaining, generated-output prompt matrix coverage, native Canvas/SVG design artifact creation, and research-memory save/clear/reload/recall through the real UI
