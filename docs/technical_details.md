...
Semantic index persistence is separate derived state under the configured `semanticIndexFolder`, defaulting to `Agent Memory`. Defaults are enabled, 3000 ms debounce, 1000 files, 512 embedding dimensions, and persisted local vectors in JSON. Changing the model, dimension, chunking, or vector-persistence setting makes existing index JSON stale until rebuilt or updated. The settings UI exposes semantic search, embedding, chunking, Python/cache, and index maintenance options, but the model-facing tools still hide raw vectors.
...
- runs isolated Playwright scenarios for clear-chat prompt typing, append, prior-assistant writeback, streaming writeback, folder traversal, semantic index inspection plus note search, autonomous loop depths, checkpoint creation during long autonomous runs, prompt-on-page traversal, current-page replacement, explicit CRUD chaining, generated-output prompt matrix coverage, native Canvas/SVG design artifact creation, and research-memory save/clear/reload/recall through the real UI
