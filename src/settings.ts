...
export const DEFAULT_SETTINGS: AgentSettings = {
  ...
  specialistModel: "qwen3.5:cloud",
  utilityModel: "qwen3.5:cloud",
...
export class AgentSettingTab extends PluginSettingTab {
  ...
  private renderBasicSection(containerEl: HTMLElement): void {
    ...
    if (provider === "ollama") {
      ...
      modelSetting.descEl.createSpan({
        text: ". The current pipeline recommendation is glm-5.3-flash:cloud; paste another exact tag or apply the recommendation, then Test connection.",
      });
    } else {
    ...
    modelSetting.addText((text) => {
      ...
      return text
        .setPlaceholder(
          provider === "ollama" ? "e.g. glm-5.3-flash:cloud" : "e.g. gpt-4o-mini",
        )
    ...
    if (provider === "ollama") {
      modelSetting.addButton((button) =>
        button
          .setButtonText("Use glm-5.3-flash:cloud")
          .setTooltip("Apply the current measured-best Cloud lead model")
          .onClick(async () => {
            this.plugin.settings.model = "glm-5.3-flash:cloud";
            this.plugin.settings.ollamaBaseUrl = "https://ollama.com/api";
            this.plugin.invalidateModelConnectionStatus();
            await this.plugin.saveSettings();
            new Notice("Ollama Cloud lead model glm-5.3-flash:cloud applied. Add your API key, then Test connection.");
            this.display();
          }),
      );
    }
      ...
      dropdown.onChange(async (value) => {
        ...
        new Notice(
          preset.provider === "ollama"
            ? `${preset.label} endpoint applied. The current model tag is preserved; use glm-5.3-flash:cloud above for the recommended lead model, then Test connection.`
