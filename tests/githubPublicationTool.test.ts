  ...
  const tool = createGitHubPublicationTool({
    ...
    createWorkflow() {
      return {
        async publishDraft(request: PublishVerifiedCodeRequestV1) {
          published += 1;
          assert.equal(request.title, "Verified change for fixture");
          assert.match(request.body, /generated by the host from an already verified local commit/i);
          assert.match(request.body, /Repository profile: `fixture`/);
          assert.match(request.body, new RegExp(`Commit: \`${GIT_B}\``));
          return {
...
test("GitHub draft publication still rejects an explicitly blank body", async () => {
  let published = 0;
  const tool = createGitHubPublicationTool({
    async resolveHandoff() {
      return verifiedHandoff();
    ...
    createWorkflow() {
      return {
        async publishDraft() {
          published += 1;
          return checkpoint("draft_pr_verified");
        },
      } as never;
    },
    async persistExternalReceipt() {},
  ...
  await assert.rejects(
    tool.executeResult!({
      action: "publish_draft",
      profileKey: "fixture",
      title: "Explicit blank body must remain invalid",
      body: "   ",
    }, context()),
    /pull request body must contain/i,
  );
...
test("publish_draft retries a readback-proven missing draft PR without resetting verified push lineage", async () => {
  const blocked = {
    ...checkpoint("blocked"),
    completionProof: "draft_pr" as const,
    pullRequest: null,
    ...
    blocker: {
      code: "github_draft_pr_not_applied",
      message: "Exact readback found no draft pull request.",
    },
  };
  let retried = 0;
  let republished = 0;
    ...
    createWorkflow() {
      return {
        async retryDraftPublicationAfterNotApplied(
          value: GitHubPublicationCheckpointV1,
          input: { title: string; body: string },
        ) {
          retried += 1;
          assert.equal(value.status, "blocked");
          assert.deepEqual(value.receiptIds, ["receipt-push"]);
          assert.equal(input.title, "Retry exact draft PR only");
          assert.equal(input.body, "Preserve the verified push receipt and request a fresh draft approval.");
          return {
            ...checkpoint("draft_pr_verified"),
            completionProof: "draft_pr" as const,
        ...
        async publishDraft() {
          republished += 1;
          throw new Error("must not replay the verified push");
        },
      } as never;
    },
    async persistExternalReceipt() {},
  ...
  const result = await tool.executeResult!(
    {
      action: "publish_draft",
      profileKey: "fixture",
      title: "Retry exact draft PR only",
      body: "Preserve the verified push receipt and request a fresh draft approval.",
    },
    context(),
  );
  ...
  assert.deepEqual(
    (result.output as GitHubPublicationCheckpointV1).receiptIds,
    ["receipt-push", "receipt-pr"],
  );
});
...
test("unfinished compound finalization states fail closed without minting an outer success receipt", async (t) => {
  const statuses = [
    "waiting_linear",
    "waiting_linear_link",
    "linear_linked",
  ...
  for (const status of statuses) {
    await t.test(status, async () => {
      ...
      await assert.rejects(
        ...
        (error: unknown) =>
          error instanceof ToolExecutionError &&
          error.code === "github_publication_finalization_pending" &&
          error.mutationState === "may_have_applied",
      );
...
test("retry resumes a pending compound finalization checkpoint without republishing", async () => {
  const pending = {
    ...checkpoint("waiting_linear_link"),
    completionProof: "draft_pr" as const,
  };
  let resumes = 0;
  let publishes = 0;
    ...
    createWorkflow() {
      return {
        async resumeFinalization() {
          ...
          return resumes === 1
            ? pending
            : { ...pending, status: "finalized" as const };
        },
        async publishDraft() {
          publishes += 1;
          throw new Error("must not republish a durable draft pull request");
        },
      } as never;
    },
    async persistExternalReceipt() {
...
test("proof drift after finalization resume cannot masquerade as publication success", async () => {
  const pending = {
    ...checkpoint("waiting_linear_link"),
    completionProof: "draft_pr" as const,
  };
        ...
        async resumeFinalization() {
          return {
            ...
            blocker: {
              ...
              message: "The pull request proof drifted before finalization.",
            },
          };
        },
        async publishDraft() {
...
test("merge refuses a finalized draft-only checkpoint without merge proof", async () => {
  const draftOnly = {
    ...checkpoint("finalized"),
    completionProof: "draft_pr" as const,
    mergeSha: null,
    pullRequest: {
      ...checkpoint("finalized").pullRequest!,
      merged: false,
    },
  };
  let persisted = 0;
...
test("GitHub publication intent is explicit and does not match ordinary GitHub reading", () => {
  ...
  assert.equal(
    hasExplicitGitHubPublicationIntent(
      "Do not merge it. Publish the tested commit to GitHub as one open draft pull request.",
    ),
    true,
  );
  assert.equal(
    hasExplicitGitHubPublicationIntent(
      "Publish the verified branch to GitHub as a draft pull request without merging it.",
    ),
    true,
  );
  assert.equal(
    hasExplicitGitHubPublicationIntent(
      "Without merging, publish the tested commit to GitHub as a draft PR.",
    ),
    true,
  );
});
